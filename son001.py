...
while cap.isOpened():
        ...
        if results_pose.pose_landmarks:
            ...
            if left_ear.visibility > 0.5 and right_ear.visibility > 0.5:
                ...
                if ear_distance_pixel > 0:
                    ...
                    left_x3d, left_y3d, left_z3d = left_ear.x * crop_img.shape[1], left_ear.y * crop_img.shape[0], left_ear.z * crop_img.shape[1]
                    right_x3d, right_y3d, right_z3d = right_ear.x * crop_img.shape[1], right_ear.y * crop_img.shape[0], right_ear.z * crop_img.shape[1]
    ...
    if results_pose.pose_landmarks:
        ...
        points_3d = []
        for lm in landmarks:
            x3d, y3d, z3d = lm.x, lm.y, lm.z  # MediaPipe'in 3D koordinatları
            points_3d.append([x3d, -y3d, -z3d])  # Open3D için koordinat düzeltmesi

        # Open3D nokta bulutunu güncelle
